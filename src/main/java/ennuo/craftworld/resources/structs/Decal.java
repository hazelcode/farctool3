...
public class Decal {
    ResourcePtr texture = new ResourcePtr(null, ResourceType.TEXTURE);
    public float u = 0, v = 1, xvecu = 0, xvecv = 1, yvecu = 1, yvecv = 1;
    int packedCol = 44373;
    int type = 0;
    short metadataIndex = -1, numMetadata, placedBy = -1;
    int scorchMark = 0;
    ResourcePtr plan = new ResourcePtr(null, ResourceType.PLAN);
    
    public Decal(Data data) {
        texture = data.resource(ResourceType.TEXTURE);
        u = data.float32();
        v = data.float32();
        xvecu = data.float32();
        xvecv = data.float32();
        ...
        plan = data.resource(ResourceType.PLAN);
    }
